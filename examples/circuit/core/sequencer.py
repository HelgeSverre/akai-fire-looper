...
class Sequencer:
        ...
        try:
            with self.lock:
                ...
                for track_idx, track in enumerate(self.tracks):
                    ...
                    # Get the step within the pattern
                    pattern_step = step % current_pattern.length
                    step_data = current_pattern.get_step(pattern_step)

                    if step_data.has_notes():
                        self._play_step(track, step_data, step)
        ...
        for track_idx, track in enumerate(self.tracks):
            if track.is_pattern_chained():
                ...
                if chain_pos < len(track.pattern_chain):
                    ...
                    if self.pattern_chain_step_counts[track_idx] >= pattern.length:
                        ...
                        if self.pattern_chain_positions[track_idx] >= len(
                            track.pattern_chain
                        ):
                            self.pattern_chain_positions[track_idx] = 0

    def _play_step(self, track: Track, step, global_step: int):
        ...
        for note, velocity in step.get_notes_with_velocities():
            ...
            gate_duration = step.gate_length * self.timing.get_step_duration(
                global_step
            )
            note_off_time = play_time + gate_duration

            # Store for note-off scheduling
    ...
    def launch_scene(self, scene_index: int):
        """Launch a scene (apply its pattern assignments)."""
        if 0 <= scene_index < len(self.scenes):
            scene = self.scenes[scene_index]
            scene.apply_to_tracks(self.tracks)
            self.current_scene = scene_index

            # Reset pattern chain positions when launching scene
            self.pattern_chain_positions = [0] * 4
            self.pattern_chain_step_counts = [0] * 4
