...
class ComprehensiveTest:
    ...
    def _setup_screens(self):
        ...
        self.screen_manager.add_screen(
            "pad_test", TextScreen(self.fire.get_canvas(), "Pad Test")
        )
        self.screen_manager.add_screen(
            "button_test", TextScreen(self.fire.get_canvas(), "Button Test")
        )
        self.screen_manager.add_screen(
            "rotary_test", TextScreen(self.fire.get_canvas(), "Rotary Test")
        )
        self.screen_manager.add_screen(
            "led_test", TextScreen(self.fire.get_canvas(), "LED Test")
        )

        # Progress screen
        self.screen_manager.add_screen(
            "progress", ProgressScreen(self.fire.get_canvas(), "Progress")
        )

        # Grid screen for pad visualization
        self.screen_manager.add_screen(
            "pad_grid", GridScreen(self.fire.get_canvas(), "Pad Activity", 4, 16)
        )

        # Value screens for rotaries
        self.screen_manager.add_screen(
            "volume", ValueScreen(self.fire.get_canvas(), "Volume", 0, "%")
        )
        self.screen_manager.add_screen(
            "pan", ValueScreen(self.fire.get_canvas(), "Pan", 0, "")
        )
        self.screen_manager.add_screen(
            "filter", ValueScreen(self.fire.get_canvas(), "Filter", 0, "Hz")
        )
        self.screen_manager.add_screen(
            "resonance", ValueScreen(self.fire.get_canvas(), "Resonance", 0, "%")
        )
    ...
    def _setup_event_handlers(self):
        ...
        def handle_pad(pad_idx, velocity):
            ...
            if velocity > 0:
                ...
                self.fire.set_pad_color(
                    pad_idx, brightness, brightness // 2, brightness // 4
                )
            else:
        ...
        def handle_browser(event):
            if event == "press":
                self.fire.set_button_led(
                    self.fire.BUTTON_BROWSER, self.fire.LED_DULL_GREEN
                )
        ...
        def handle_rotary(rotary_id, direction, velocity):
            ...
            rotary_values[rotary_id] = max(
                0, min(127, rotary_values[rotary_id] + delta)
            )
    ...
    def test_buttons(self):
        ...
        text_screen.set_lines(
            ["Press any button!", "LED will light up", "", "Browser = Menu"]
        )
