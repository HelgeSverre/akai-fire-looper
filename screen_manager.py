...
class MenuScreen(Screen):
    ...
    def add_item(self, label: str, value: Any = None, action: Optional[Callable] = None):
        """Add a menu item."""
...
class ProgressScreen(Screen):
    ...
    def __init__(self, canvas, title: str = "", min_val: float = 0, max_val: float = 100):
        super().__init__(canvas)
        self.title = title
    ...
    def render(self):
        ...
        if self.max_val > self.min_val:
            ...
            if fill_width > 2:
                self.canvas.fill_rect(bar_x + 1, bar_y + 1, fill_width - 2, bar_height - 2)
