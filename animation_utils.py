...
class CircularPulsePattern(AnimationPattern):
    ...
    def render(self) -> List[Tuple[int, int, int, int]]:
        pad_colors = []
        radius = (
            math.sin(self.state.time * self.frequency * 2 * math.pi) + 1
        ) * self.max_radius
...
class MatrixRainPattern(AnimationPattern):
    ...
    def __init__(
        self, drop_speed: float = 2.0, color: Tuple[int, int, int] = (0, 127, 0)
    ):
        super().__init__()
    ...
    def render(self) -> List[Tuple[int, int, int, int]]:
        ...
        for i in range(64):
            ...
            if drop_pos < 64:
                ...
                row_pos = y * 16
                intensity = max(
                    0, min(127, int(127 * (1 - abs(row_pos - drop_pos) / 16)))
                )
...
class WavePattern(AnimationPattern):
    ...
    def render(self) -> List[Tuple[int, int, int, int]]:
        ...
        for i in range(64):
            ...
            wave = math.sin(
                self.state.time * self.wave_speed * 2 * math.pi
                + pos * self.frequency * 2 * math.pi
            )
