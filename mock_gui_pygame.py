...
class PygameCanvas:
    ...
    def draw_circle(self, x0: int, y0: int, radius: int, color: int = 0):
        """Draw a circle."""
        self.draw.ellipse([x0 - radius, y0 - radius, x0 + radius, y0 + radius], outline=color)
    ...
    def draw_value_page(self, title: str, value: str, label: str = "", unit: str = "", show_bar: bool = False, bar_value: float = 0.0):
    ...
    def draw_split_screen(self, title: str, left_content: list[str], right_content: list[str]):
        """Draw split screen."""
        self.clear()
...
class MockAkaiFire:
    ...
    CONTROL_BANK_CHANNEL_MIXER_USER1 = FIELD_BASE | FIELD_CHANNEL | FIELD_MIXER | FIELD_USER1
    CONTROL_BANK_CHANNEL_MIXER_USER2 = FIELD_BASE | FIELD_CHANNEL | FIELD_MIXER | FIELD_USER2
    CONTROL_BANK_CHANNEL_USER1_USER2 = FIELD_BASE | FIELD_CHANNEL | FIELD_USER1 | FIELD_USER2
    CONTROL_BANK_MIXER_USER1_USER2 = FIELD_BASE | FIELD_MIXER | FIELD_USER1 | FIELD_USER2
    CONTROL_BANK_ALL_ON = FIELD_BASE | FIELD_CHANNEL | FIELD_MIXER | FIELD_USER1 | FIELD_USER2
    ...
    def _create_layout(self):
        ...
        rotary_y = base_y + 30
        self.rotary_data = {
            self.ROTARY_VOLUME: {"pos": (base_x + 190, rotary_y), "value": 64, "name": "VOLUME"},
            self.ROTARY_PAN: {"pos": (base_x + 280, rotary_y), "value": 64, "name": "PAN"},
            self.ROTARY_FILTER: {"pos": (base_x + 370, rotary_y), "value": 64, "name": "FILTER"},
            self.ROTARY_RESONANCE: {"pos": (base_x + 460, rotary_y), "value": 64, "name": "RESONANCE"},
            self.ROTARY_SELECT: {"pos": (base_x + 750, base_y + 140), "value": 64, "name": "SELECT"}
        }
        
        # Buttons
        # Left side - control bank
        self.button_rects[self.BUTTON_BANK] = pygame.Rect(base_x + 20, base_y + 120, 35, 18)
        self.button_rects[self.BUTTON_SELECT] = pygame.Rect(base_x + 60, base_y + 120, 40, 18)
        ...
        self.button_rects[self.BUTTON_PAT_UP] = pygame.Rect(base_x + 630, base_y + 10, 25, 18)
        self.button_rects[self.BUTTON_PAT_DOWN] = pygame.Rect(base_x + 630, base_y + 32, 25, 18)
        self.button_rects[self.BUTTON_PATTERN] = pygame.Rect(base_x + 660, base_y + 10, 50, 40)
        
        # Browser
        self.button_rects[self.BUTTON_BROWSER] = pygame.Rect(base_x + 720, base_y + 10, 50, 25)
        
        # Grid navigation
        self.button_rects[self.BUTTON_GRID_LEFT] = pygame.Rect(base_x + 700, base_y + 140, 30, 20)
        self.button_rects[self.BUTTON_GRID_RIGHT] = pygame.Rect(base_x + 770, base_y + 140, 30, 20)
        
        # Bottom row - mode buttons
        bottom_y = base_y + 250
        self.button_rects[self.BUTTON_STEP] = pygame.Rect(base_x + 180, bottom_y, 38, 20)
        self.button_rects[self.BUTTON_NOTE] = pygame.Rect(base_x + 222, bottom_y, 38, 20)
        self.button_rects[self.BUTTON_DRUM] = pygame.Rect(base_x + 264, bottom_y, 38, 20)
        self.button_rects[self.BUTTON_PERFORM] = pygame.Rect(base_x + 306, bottom_y, 48, 20)
        
        # Shift/Alt
        self.button_rects[self.BUTTON_SHIFT] = pygame.Rect(base_x + 420, bottom_y, 38, 20)
        self.button_rects[self.BUTTON_ALT] = pygame.Rect(base_x + 462, bottom_y, 30, 20)
        
        # Transport
        self.button_rects[self.BUTTON_PLAY] = pygame.Rect(base_x + 650, bottom_y, 38, 20)
        self.button_rects[self.BUTTON_STOP] = pygame.Rect(base_x + 692, bottom_y, 38, 20)
    ...
    def _draw(self):
        ...
        for button_id, rect in self.button_rects.items():
            ...
            if button_id in button_labels:
                font = self.tiny_font if len(button_labels[button_id]) > 4 else self.small_font
                text = font.render(button_labels[button_id], True, (180, 180, 180))
                text_rect = text.get_rect(center=rect.center)
                self.screen.blit(text, text_rect)
                
        # Grid label
        grid_x = (self.button_rects[self.BUTTON_GRID_LEFT].right + 
                  self.button_rects[self.BUTTON_GRID_RIGHT].left) // 2
    ...
    def _draw_oled(self, x, y):
        ...
        pygame.draw.rect(self.screen, (60, 60, 60), frame_rect, width=1, border_radius=2)
