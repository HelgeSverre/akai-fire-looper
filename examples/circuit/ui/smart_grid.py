...
class PadData:
    ...
    def __hash__(self):
        ...
        return hash((
            self.state, self.track, self.step, self.pattern, 
            self.velocity, self.is_current_step, self.is_selected
        ))
...
class SmartGridManager:
    ...
    def _dim_color(self, color: Tuple[int, int, int], brightness: int) -> Tuple[int, int, int]:
    ...
    def _update_note_mode_state(self, status: Dict[str, Any], **kwargs):
        ...
        for col in range(16):
            ...
            if col < 4:  # Track selection
                pad.state = PadState.HAS_CONTENT if col == current_track_idx else PadState.EMPTY
                pad.track = col
    ...
    def _step_has_content(self, track_idx: int, pattern_idx: int, step: int, tracks) -> bool:
    ...
    def _apply_layers(self, base_color: Tuple[int, int, int], pad: PadData) -> Tuple[int, int, int]:
